...
mode_pad_dict = {"s": 4, "d": 4, "y": 4, "e": 3, "h": 2, "j": 1, "o": 3, "f": 4}
...
def valid_steps(model_G, valid, opt, iter):
    ...
    with torch.no_grad():
        ...
        for i in range(len(datasets)):
            ...
            logger.info(
                'Iter {} | Dataset {} | AVG Val PSNR: {:02f}, AVG: SSIM: {:04f}'.format(iter, datasets[i], np.mean(np.asarray(psnrs)), np.mean(np.asarray(ssims))))
